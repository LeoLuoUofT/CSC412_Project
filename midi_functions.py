...
def programs_to_instrument_matrix(programs, instrument_attach_method,
                                  max_voices):
...
def rolls_to_midi(pianoroll, programs, save_folder, filename, bpm,
                  velocity_roll=None, held_notes_roll=None):
    ...
    for voice, program in enumerate(programs):
        ...
        if velocity_roll is not None:
            ...
            current_velocity_roll[np.where(
                current_velocity_roll < velocity_threshold_such_that_it_is_a_played_note)] = 0
            current_velocity_roll[np.where(
                current_velocity_roll >= velocity_threshold_such_that_it_is_a_played_note)] -= 0.5
        ...
        for i, note_vector in enumerate(current_pianoroll):
            ...
            for note in tracker:
                ...
                if hold_this_note:
                    ...
                    midi_note = pm.Note(velocity=velocity, pitch=note,
