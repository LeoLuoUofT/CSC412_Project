...
def load_rolls(path, name):
    ...
    for instrument in mid.instruments:
        ...
        for note in instrument.notes:
            ...
            if decimal < 10e-3 or absolute_end - absolute_start >= 1:
                ...
                note_to_velocity_dict[
                    (absolute_start, note.pitch)] = note.velocity
                note_to_duration_dict[(
                absolute_start, note.pitch)] = absolute_end - absolute_start
        ...
        for step, note_vector in enumerate(piano_roll):
            ...
            for voice_number, pitch in enumerate(
                ...
                if (step, pitch) in note_to_velocity_dict.keys():
                    velocity_roll[step, voice_number] = note_to_velocity_dict[
                        (step, pitch)]
    ...
    if print_anything:
        print(max_concurrent_notes_per_track_list)
    override_max_notes_per_track_list = [MAXIMAL_NUMBER_OF_VOICES_PER_TRACK for
                                         _ in
                                         max_concurrent_notes_per_track_list]
    silent_tracks_if_we_dont_override = max_voices - sum(
        [min(MAXIMAL_NUMBER_OF_VOICES_PER_TRACK, x) if x > 0 else 0 for x in
         max_concurrent_notes_per_track_list[:max_voices]])

    if print_anything:
        print("Silent tracks if no override: ",
              silent_tracks_if_we_dont_override)
    for voice in range(
            min(max_voices, len(max_concurrent_notes_per_track_list))):
        if silent_tracks_if_we_dont_override > 0 and \
                max_concurrent_notes_per_track_list[
                    voice] > MAXIMAL_NUMBER_OF_VOICES_PER_TRACK:
        ...
        if max_concurrent_notes > 0:
            ...
            if include_only_monophonic_instruments:
                if max_concurrent_notes > 1:
                    if print_anything:
                        print(
                            "Skipping this piano roll since it's polyphonic. Program number ",
                            program)
                    continue
                else:
                    if print_anything:
                ...
                for voice in range(min(max_concurrent_notes,
                                       max(MAXIMAL_NUMBER_OF_VOICES_PER_TRACK,
    ...
    if len(chosen_piano_rolls) > 0:
        ...
        instrument_feature_matrix = mf.programs_to_instrument_matrix(
            chosen_programs, instrument_attach_method, max_voices)
        ...
        if save_preprocessed_midi:
            mf.rolls_to_midi(Y, chosen_programs,
                             'preprocess_midi_data/' + t + '/', name, tempo, V,
                             D)
        ...
        if output_length > 0:
            ...
            # pad to the right..
            Y = np.pad(Y, ((0, padding_length), (0, 0)), 'constant',
                       constant_values=(0, 0))
            if include_silent_note:
...
def import_midi_from_folder(folder):
    ...
    if load_from_pickle_instead_of_midi:
        ...
        train_paths = pickle.load(
            open(pickle_load_path + "train_paths.pickle", 'rb'))
        test_paths = pickle.load(
            open(pickle_load_path + "test_paths.pickle", 'rb'))
    ...
    if split_equally_to_train_and_test:
        ...
        V_train, V_test, D_train, D_test, T_train, T_test, I_train, I_test, Y_train, Y_test, X_train, X_test, c_train, c_test, train_paths, test_paths = train_test_split(
            V_list, D_list, T_list, I_list, Y_list, X_list, c_classes, paths,
            test_size=test_fraction, random_state=42, stratify=c_classes)
        ...
        if shuffle:
            ...
            for i in range(int(len(X_array) / chunks)):
                permutation.extend(
                    range(i, len(X_array), int(len(X_array) / chunks)))
    ...
    if equal_mini_songs:
        ...
        for i, song in enumerate(X_train):
            c = c_train[i]
            splits_per_class[c] += math.ceil(
                len(song) / (output_length // max_voices))
        ...
        for i, song in enumerate(X_train):
            c = c_train[i]
            if splits_per_class_new[c] + math.ceil(len(song) / (
                    output_length // max_voices)) <= amount_of_splits:
                ...
                splits_per_class_new[c] += math.ceil(
                    len(song) / (output_length // max_voices))
        if print_anything:
            print(splits_per_class_new)
    ...
    if save_imported_midi_as_pickle:
        ...
        pickle.dump(train_paths,
                    open(pickle_store_folder + 'train_paths.pickle', 'wb'))
        pickle.dump(test_paths,
                    open(pickle_store_folder + 'test_paths.pickle', 'wb'))
